...
export type VisionObservationResult = {
  ...
  detected_subjects: Array<{
    subject_id: string;
    type: "person" | "outfit" | "object" | "other";
    visibility: string;
    region: Region;
    confidence: number;
  }>;
...
export const visionObservationJsonSchema = {
  type: "object",
  additionalProperties: false,
  required: [
    "image_summary",
  ...
  properties: {
    ...
    image_quality: {
      ...
      properties: {
        ...
        angle: {
          type: "string",
          enum: ["front", "side", "mirror", "unclear"],
        },
        occlusion: {
          type: "string",
    ...
    detected_subjects: {
      ...
      items: {
        ...
        properties: {
          ...
          type: {
            type: "string",
            enum: ["person", "outfit", "object", "other"],
          },
          visibility: {
            type: "string",
          },
...
const subjectTypes = new Set(["person", "outfit", "object", "other"]);
...
export function parseVisionObservationResult(value: unknown): VisionObservationResult {
  ...
  if (
    ...
    value.visual_observations.length > 8
  ) {
    throw new Error("Visual observations must contain between 3 and 8 items.");
  }
  ...
  const detectedSubjects = value.detected_subjects.map((subject) => {
    if (!isObject(subject)) {
      throw new Error("Invalid detected subject.");
    }

    if (
      !isString(subject.subject_id) ||
      !subjectTypes.has(subject.type as string) ||
    ...
    return {
      ...
      type: subject.type as VisionObservationResult["detected_subjects"][number]["type"],
  ...
  const visibleItems = value.visible_items.map((item) => {
    ...
    if (
      ...
      !itemCategories.has(item.category as string) ||
    ...
    return {
      ...
      category: item.category as VisionObservationResult["visible_items"][number]["category"],
  ...
  const visualObservations = value.visual_observations.map((observation) => {
    ...
    if (
      ...
      !observationTypes.has(observation.observation_type as string) ||
    ...
    return {
      ...
      observation_type:
        observation.observation_type as VisionObservationResult["visual_observations"][number]["observation_type"],
  ...
  return {
    ...
    image_quality: {
      ...
      confidence: imageQuality.confidence,
    },
